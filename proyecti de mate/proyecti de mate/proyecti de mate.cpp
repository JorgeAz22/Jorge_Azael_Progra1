...
int main() {
    ...
    do {
        ...
        Pokemon maquina("Haunter", 100, 25);
        ...
        while (jugador.estaVivo() && maquina.estaVivo()) {
            if (turno % 2 == 1) {
                ...
                if (opcion == 1) {
                    ...
                    int probabilidadEsquivar = rand() % 100 + 1; // Número aleatorio entre 1 y 100
                    const int PROBABILIDAD_ESQUIVAR = 30; // Probabilidad de esquivar: 30%

                    bool esquivado = probabilidadEsquivar <= PROBABILIDAD_ESQUIVAR;
                    if (esquivado) {
                        std::cout << "¡Has esquivado el ataque del oponente!" << std::endl;
                    }
                    else {
